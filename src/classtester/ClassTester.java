...
public class ClassTester {
    ...
    private ClassLoader myClassPath;
    private Class myClass;

    private Method[] methodsArr;
     ...
     * @throws Exception 异常
     */
    public String[] getClassMethods(String javaPath) throws Exception {
...
*/
     ...
     * @throws Exception 异常
     */
    public void setCurrentMethod(int MethodID) throws Exception {
     ...
     * @throws Exception 异常
     */
    public String runTest(String testFile, int[] paramPos, int[] resultPos, int[] returnPos, int isTruePos) throws Exception {
...
//                System.out.println(sheet.getCell(paramPos[j], i).getContents());
                switch (paramArr[j]){
                    case "int":
                        valueArr[j] = Integer.parseInt(sheet.getCell(paramPos[j], i).getContents());break;
                    case "double":
                        valueArr[j] = Double.parseDouble(sheet.getCell(paramPos[j], i).getContents());break;
                    case "float":
                        valueArr[j] = Float.parseFloat(sheet.getCell(paramPos[j], i).getContents());break;
                    case "char":
                        valueArr[j] = sheet.getCell(paramPos[j], i).getContents().charAt(0);break;
                    case "byte":
                        valueArr[j] = Byte.parseByte(sheet.getCell(paramPos[j], i).getContents());break;
                    case "short":
                        valueArr[j] = Short.parseShort(sheet.getCell(paramPos[j], i).getContents());break;
                    case "long":
                        valueArr[j] = Long.parseLong(sheet.getCell(paramPos[j], i).getContents());break;
                    case "boolean":
                        valueArr[j] = Boolean.parseBoolean(sheet.getCell(paramPos[j], i).getContents());break;
                    default:
                        valueArr[j] = sheet.getCell(paramPos[j], i).getContents();break;
                }
            }
